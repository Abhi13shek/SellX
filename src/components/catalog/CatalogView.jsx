...
import { Search, X, Check, ShoppingCart, ArrowLeftRight, SlidersHorizontal } from "lucide-react";
...
export function CatalogView({ products, onRequestQuote, onToggleCart, cartIds, onOpenProduct }) {
  ...
  const visibleProducts = useMemo(() => {
    let list = products.filter((p) => {
      const matchesSearch =
        ...
        (p.description && p.description.toLowerCase().includes(search.trim().toLowerCase()));
  ...
  return (
    <div className="sellx-rise space-y-5">
      {/* Sleek unified Search & Filter Bar */}
      <div className="flex flex-col md:flex-row items-stretch md:items-center justify-between gap-3">
        {/* Search input */}
        <div className="relative flex-1 flex items-center bg-[var(--surface)] border border-[var(--line)] rounded-xl px-3.5 shadow-sm focus-within:border-[var(--teal)] transition-colors">
        ...
        <div className="flex items-center gap-2 shrink-0">
          <div className="flex items-center gap-1.5 px-3 py-2 bg-[var(--surface)] border border-[var(--line)] rounded-xl text-xs font-medium text-[var(--mist)]">
      ...
      <div className="flex items-center gap-2 overflow-x-auto pb-1 scrollbar-none">
        {categories.map((cat) => {
          ...
          const c = cat === "All" ? "var(--teal)" : catColor(cat);
          return (
            <button
              key={cat}
